...
bool solution(const char* password) {
	...
	for (int i = 0; i < strlen(password); i++) {
		if (password[i] >= 'a' && password[i] <= 'z') small_count++;
		else if (password[i] >= 'A' && password[i] <= 'Z') digit_count++;
		else if (password[i] >= '1' && password[i] <= '9') capital_count++;
	}

	if (capital_count * small_count * digit_count != 0) answer = true;
	else answer = false;
	return answer;
}
